---
title: "Markos et al Transitions graphs"
...
RELOAD_TM=TRUE # If TRUE, provided transition matrix is used rather than the computed one for time saving purposes.

```

# Preprocess

Load data and select Ectoderm Elav-positive cells:

Using provided precomputed transition probabilities: `r RELOAD_TM`

```{r}
if(RELOAD_TM){
  tm <- readRDS('../Results/tm_original.RDS')
} else {
...
sc_int$Elav_plus <- assay(sc_int, 'logcounts')['Elav', ] > 0
urd_o@meta$celltype[urd_o@meta$celltype == 'Ectoderm' & sc_int$Elav_plus] <- 'Ectoderm_Elav'
