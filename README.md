...
*Important: To reduce compressed repository size for submission, input files were omitted and labeled ON DEMAND in the content listing below. To retrieve these files, please contact corresponding author.*
...
- **10X_matrices**
    ...
    - **G4; N0; N2; N5** directories: outputs of 10X cellranger count pipeline (filtered expression matrices), inputs for the individual timepoints analyses *ON DEMAND*
...
- **Results**
    ...
    - **Fig1_*_original.pdf**: output of `R_demo.sh`
    - **tm_original.RDS**: precomputed transition probabilities matrix *ON DEMAND*
- **Export**
    ...
    - **Transition_matrix_celltype_timepoint.xlsx**: resulting transition matrix in different transformations, each in own sheet with description (formatted in spreadsheet editor) *ON DEMAND*
